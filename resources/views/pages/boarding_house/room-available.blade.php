...
@section('content')
    ...
    <form action="{{ route('booking', $item->slug) }}" class="relative flex flex-col gap-4 mt-5">
        <input type="hidden" name="boarding_house_id" value="{{ $item->id }}">
        <h2 class="font-bold px-5">Available Rooms</h2>
        <div id="RoomsContainer" class="flex flex-col gap-4 px-5">
            @foreach ($item->rooms as $room)
                <label class="relative group">
                    <input type="radio" name="room" class="absolute top-1/2 left-1/2 -z-10 opacity-0"
                    ...
                    <div
                        ...
                        <div class="flex flex-col gap-3 w-full">
                            <h3 class="font-semibold text-lg leading-[27px]">
                                {{ $room->name }}
                            </h3>
                            <hr class="border-[#F1F2F6]">
                            <div class="flex items-center gap-[6px]">
                            ...
                            <div class="flex items-center gap-[6px]">
                                <img src="{{ asset('assets/images/icons/3dcube.svg') }}" class="w-5 h-5 flex shrink-0"
                                    alt="icon">
                                <p class="text-sm text-ngekos-grey">
                                    {{ $room->square_feet }} sqft flat
                                </p>
                            </div>
                            <hr class="border-[#F1F2F6]">
                            <p class="font-semibold text-lg text-ngekos-orange">
                                {{ Number::currency($room->price_per_month, 'IDR') }}
                                <span class="text-sm text-ngekos-grey font-normal">
                                    {{ $room->boardingHouse->category->name === 'Kost' ? '/bulan' : '/hari' }}
                                </span>
                            </p>
                        </div>
                    </div>
